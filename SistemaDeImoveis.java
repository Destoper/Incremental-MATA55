...
public class SistemaDeImoveis {
    ...
    public boolean bloquearImovel( String numeroIPTU, String data) throws  NaoExistemException, NaoCadastradoException {
        ...
        for (Imovel imovel : this.imoveis) {
            if (imovel.getNumeroIPTU().equals(numeroIPTU)) {
               return imovel.adicionarDataBloqueadaAgenda(data);
            }
        }
        throw new NaoCadastradoException("Imóvel não cadastrado");
    }

    public boolean alugarImovel( String numeroIPTU, String data) throws  NaoExistemException, NaoCadastradoException {
        if (this.imoveis.isEmpty()) {
            throw new NaoExistemException("Não existem imóveis cadastrados");
        }
        ...
        for (Imovel imovel : this.imoveis) {
            if (imovel.getNumeroIPTU().equals(numeroIPTU)) {
                return imovel.adicionarDataAlugadaAgenda(data);
            }
        }
        throw new NaoCadastradoException("Imóvel não cadastrado");
    }
